...
class VNSeqMInplace(BaseSchema):
    ...
    def __init__(self, dataset_size: int, scene, N: int = 20, M: int = 1, num_init_views: int = 4, interval_epochs=1000, **kwargs):
        """
        ...
        if num_views_left > 0:
            ...
            while num_views_left > 0:
                ...
                it_base+= cur_dataset_size * interval_epochs
                num_views_left -= M
        print('camera planning schedule', self.load_its)


V20Seq1Inplace = partial(VNSeqMInplace, N=20, M=1, num_init_views=4)
V10Seq1Inplace = partial(VNSeqMInplace, N=10, M=1, num_init_views=2,interval_epochs=300)
V20Seq4Inplace = partial(VNSeqMInplace, N=20, M=4, num_init_views=4)
